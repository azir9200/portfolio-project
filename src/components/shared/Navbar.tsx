...
const Header = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-background/70 backdrop-blur-xl border-b border-border/60 shadow-[0_8px_30px_rgba(0,0,0,0.08)] transition-colors duration-300">
      <div className="container mx-auto px-6 py-4">
          ...
          <nav className="hidden md:flex space-x-8">
            {navItems.map((item) => (
              <button
                ...
                className={`relative transition-all duration-300 font-medium focus-ring px-3 py-2 rounded-md group ${
                  activeSection === item.href.slice(1)
                    ? `text-foreground font-semibold`
                    : "text-foreground/80 hover:text-foreground"
                } after:absolute after:left-1/2 after:-bottom-1 after:h-0.5 after:-translate-x-1/2 after:rounded-full after:transition-all after:duration-300 after:w-0 ${
                  activeSection === item.href.slice(1)
                    ? `after:w-6 after:opacity-100 after:bg-gradient-to-r ${getSectionColor(
                        activeSection,
                      )}`
                    : "after:bg-foreground/40 after:opacity-0 group-hover:after:opacity-70 group-hover:after:w-6"
                }`}
              >
                {item.name}
              </button>
        ...
        {isMenuOpen && (
          <nav className="md:hidden mt-4 pb-4 space-y-2 animate-fade-in bg-background/80 backdrop-blur-xl rounded-lg border border-border/60 shadow-lg p-4">
            {navItems.map((item) => (
              <button
                key={item.name}
                onClick={() => handleNavClick(item.href)}
